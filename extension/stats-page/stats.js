...
async function loadCustomImageNames() {
  try {
    const result = await chrome.storage.local.get(['imageNames']);
...
function saveCustomImageNames() {
  chrome.storage.local.set({ imageNames: customImageNames });
}

/**
 * The name to show for an image everywhere in the UI: the user's custom name
 * if set, otherwise the collector's (prompt-derived) name.
 */
function displayName(image) {
  return customImageNames[image.id] || image.name || `Image ${image.id}`;
}

/**
 * Inline rename editor: swaps the card's name for an input; Enter/blur saves,
 * Escape cancels. Saving an empty value (or the original name) removes the
 * custom name and falls back to the collector's.
 */
function startRename(imageId, btn) {
  ...
  const original = image.name || `Image ${image.id}`;
  ...
  const finish = (save) => {
    ...
    if (save) {
      ...
      if (!value || value === original) {
        delete customImageNames[imageId];
      } else {
        customImageNames[imageId] = value;
      }
      ...
      <div class="image-card-header">
        ...
        <div class="image-info">
          <div class="image-name">
            <a href="${escapeHtml(image.url)}" target="_blank" rel="noopener" title="${escapeHtml(image.name || `Image ${image.id}`)}">
